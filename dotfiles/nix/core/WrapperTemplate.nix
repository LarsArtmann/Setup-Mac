...
let
  ...
  WrapperConfigType = lib.types.submodule {
    options = {
      ...
      wrapperName = lib.mkOption {
        type = lib.types.str;
        description = "Name for the wrapper binary";
        default = "wrapped-${config.packageName}";
        readOnly = true;
      };
    };
  };
  ...
  wrapWithConfig = config:
    let
      ...
      wrapperScript = writeShellScriptBin validatedConfig.wrapperName ''
        #!/usr/bin/env bash
      ...
      finalWrapper = symlinkJoin {
        name = validatedConfig.wrapperName;
        paths = [ validatedConfig.package wrapperScript ];
        buildInputs = [ makeWrapper ];
        postBuild = ''
          # Ensure wrapper is executable
          chmod +x $out/bin/${validatedConfig.wrapperName}
        '';
      };
