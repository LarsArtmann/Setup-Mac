{
  ...
  in lib.filter (d: d != null) domains;

  # Convert domains to unbound local-data format
  domainsToUnbound = domains: ip: lib.concatMapStringsSep "\n"
    (d: ''local-data: "${d} A ${ip}"'') domains;
in {
  ...
  in runCommand "${name}-unbound.conf" {} ''
  ...
  combineBlocklists = name: blocklists: runCommand "${name}-combined.conf" {} ''
