...
# Provides mechanism to list and manage available program modules
{
  ...
  getEnabledPrograms = enabledPrograms: let
  ...
  getEnabledPackages = enabledPrograms: let
  ...
  getEnabledModules = enabledPrograms: let
    enabledConfigs = getEnabledPrograms enabledPrograms;
  ...
  listPrograms = format: let
    ...
    formatLine = program:
      if format == "simple"
      then "  ${program.name} - ${program.description} (${program.category})"
  ...
  in
    if format == "json"
    then builtins.toJSON programList
