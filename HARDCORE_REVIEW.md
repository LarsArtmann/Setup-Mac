...
The `flake.nix` overrides the global `go` package to version `1.26.0` (which is compiled from source via a `fetchurl` tarball) and forces `buildGoModule` to use it for *every* Go package in the system.
