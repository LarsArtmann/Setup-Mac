...
# TYPE-SAFE CLI TOOL WRAPPER GENERATION
{
  ...
  basePackage =
    if validateInputs.package
    then pkgs.${packageName} or (builtins.throw "Package ${packageName} not found in nixpkgs")
    else builtins.throw "Invalid package name: ${packageName}";
  ...
  '';
in
  lib.mkIf validateInputs.allValid {
