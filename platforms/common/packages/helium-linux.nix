...
in
  pkgs.stdenv.mkDerivation rec {
    ...
    # IMPORTANT: Must not use 'with pkgs' or inline packages for isLinux checks
    # because Nix evaluates all packages for platform compatibility even in lib.optionals
    buildInputs =
      if isDarwin
      then []
      else [
    ...
    meta = with lib; {
      ...
      broken =
        (isDarwin && isx86_64 && lib.versionOlder pkgs.stdenv.hostPlatform.darwinMinVersion "10.15")
